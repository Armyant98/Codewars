...
function printerError(s) {
    console.log(s.match(/[a-m]/g).length) //regex to find the 
    
    let totalLength = s.length
    let errors = s.length - s.match(/[a-m]/g).length
    return `${errors}/${totalLength}`
    
  }

  //without regex just checks if thigs are higher than m lexicographically
